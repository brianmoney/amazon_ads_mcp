"""Internal async report orchestration for Sponsored Display tools."""
...
from .common import get_sd_client

SD_CREATE_REPORT_MEDIA_TYPE = "application/vnd.createasyncreportrequest.v3+json"
...
class SDReportError(RuntimeError):
    """Raised when the Sponsored Display report lifecycle fails."""
...
def _build_http_error(message: str, exc: httpx.HTTPError) -> SDReportError:
    ...
    if response is not None:
        ...
        if isinstance(payload, dict):
            detail = payload.get("message") or payload.get("details") or payload.get("code")
...
def _parse_report_rows(content: bytes) -> list[dict[str, Any]]:
    try:
        ...
        raise SDReportError(
            "Sponsored Display report payload could not be decompressed."
        ) from exc
    ...
    try:
        ...
        raise SDReportError(
            "Sponsored Display report payload was not valid JSON."
        ) from exc
    ...
    if not isinstance(payload, list) or not all(
        ...
        raise SDReportError(
            "Sponsored Display report payload did not contain a row list."
        )
...
def normalize_sd_report_status(raw_status: Any) -> str:
...
def _normalize_status_payload(
    ...
    return {
        ...
        "status": normalize_sd_report_status(raw_status),
...
def _format_timeout_message(report_id: str, status: dict[str, Any], elapsed: float) -> str:
    return (
        f"Sponsored Display report {report_id} timed out while polling after "
...
def _format_terminal_status_message(
    ...
    message = (
        f"Sponsored Display report {report_id} failed with status "
...
def _format_not_ready_message(report_id: str, status: dict[str, Any]) -> str:
    message = (
        f"Sponsored Display report {report_id} is not ready "
...
async def create_sd_report(
    *,
    ...
    """Create a Sponsored Display report and return its report ID."""
    client = client or await get_sd_client()
    ...
    try:
        create_response = await client.post(
            ...
            json=request_body,
            headers={
                "Content-Type": SD_CREATE_REPORT_MEDIA_TYPE,
        ...
        if response is not None and response.status_code == 425:
            ...
            if match:
                ...
                raise _build_http_error(
                    "Sponsored Display report creation failed.", exc
                ) from exc
        else:
            raise _build_http_error(
                "Sponsored Display report creation failed.", exc
    ...
    if not report_id:
        raise SDReportError(
            "Sponsored Display report creation did not return a reportId."
        )
    return str(report_id)


async def fetch_sd_report_status(report_id: str, *, client=None) -> dict[str, Any]:
    ...
    try:
        ...
        raise _build_http_error(
            f"Sponsored Display report {report_id} status lookup failed.", exc
        ) from exc

    if not isinstance(status_payload, dict):
        raise SDReportError(
            f"Sponsored Display report {report_id} returned an invalid status payload."
        )

    return _normalize_status_payload(str(report_id), status_payload)


async def wait_for_sd_report(
    report_id: str,
    *,
    ...
    """Poll a Sponsored Display report until it completes or times out."""
    client = client or await get_sd_client()
    ...
    while True:
        ...
        except SDReportError as exc:
            if last_status is None:
                raise
            elapsed = max(monotonic_func() - start_time, 0.0)
            raise SDReportError(
                f"Sponsored Display report {report_id} status lookup failed after "
        ...
        if status["status"] in {"FAILED", "CANCELLED"}:
            raise SDReportError(_format_terminal_status_message(report_id, status, elapsed))
        if now >= deadline:
            raise SDReportError(_format_timeout_message(report_id, status, elapsed))
...
async def download_sd_report_rows(
    report_id: str,
    *,
    status: dict[str, Any] | None = None,
    client=None,
) -> list[dict[str, Any]]:
    """Download and parse rows for a completed Sponsored Display report."""
    client = client or await get_sd_client()
    report_status = status or await fetch_sd_report_status(report_id, client=client)
    if report_status["status"] != "COMPLETED":
        if report_status["status"] in {"FAILED", "CANCELLED"}:
            raise SDReportError(_format_terminal_status_message(report_id, report_status, 0.0))
        raise SDReportError(_format_not_ready_message(report_id, report_status))

    download_url = report_status.get("download_url")
    if not download_url:
        raise SDReportError(
            f"Sponsored Display report {report_id} completed without a download URL."
        )
    ...
    try:
        ...
        raise _build_http_error(
            f"Sponsored Display report {report_id} download failed.", exc
        ) from exc

    return _parse_report_rows(download_response.content)


async def resume_sd_report(report_id: str, *, client=None) -> dict[str, Any]:
    """Resume a known Sponsored Display report from its existing report ID."""
    client = client or await get_sd_client()
    status = await fetch_sd_report_status(report_id, client=client)
    rows = await download_sd_report_rows(report_id, status=status, client=client)
    return {"report_id": str(report_id), "rows": rows}


async def run_sd_report(
    *,
    report_type_id: str,
    start_date: str,
    end_date: str,
    ...
    """Run a Sponsored Display report end-to-end and return parsed rows."""
    client = client or await get_sd_client()
    report_id = await create_sd_report(
        report_type_id=report_type_id,
        start_date=start_date,
        end_date=end_date,
        group_by=group_by,
    ...
    status = await wait_for_sd_report(
        report_id,
